...
/// Specifies a scalar texture file, a scalar procedural texture file or an
/// image that is used to selectively replace the material color with the
/// texture color.
///
/// During rendering, the `Ka`, `Kd`, and `Ks` values and the `map_Ka`,
/// `map_Kd`, and `map_Ks` values are blended according to the following
/// formula:
///
///     result_color = tex_color(tv) * decal(tv) + mtl_color * (1.0 - decal(tv))
///
/// Where `tv` is the texture vertex.
class DecalStatement implements MtlStatement {
  /// The name of the scalar texture file, the scalar procedural texture file or
  /// the image file.
  ...
  /// Instantiates a new [DecalStatement].
  DecalStatement(this.filename,
      {this.blendU: true,
      this.blendV: true,
  ...
  void acceptVisit(MtlStatementVisitor visitor) {
    visitor.visitDecalStatement(this);
  }
  ...
  String toString() => 'DecalStatement($filename, blendU: $blendU, blendV: '

...
class NiStatementBuilder implements MtlStatementBuilder {
  ...
  NiStatementBuilder(this.lineNumber);

  void addStringArgument(String argument) {
    if (_enforceMaxArgumentCount()) {
      _errors.add(new ArgumentTypeError(
          lineNumber, 'Ni', _argumentCount, 'String', ['int', 'double']));
    }
  ...
  MtlStatementResult build() {
    if (_argumentCount < 1) {
      _errors.add(new MtlReadingError(
          lineNumber, 'A `Ni` statement requires 1 argument.'));
    }
  ...
  bool _enforceMaxArgumentCount() {
    if (_argumentCount >= 1) {
      _errors.add(new MtlReadingError(
          lineNumber, 'A `Ni` statement only takes 1 argument.'));
